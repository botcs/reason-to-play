...
from vgdl.core import GameState, Action
    ...
    def grow_graph_bfs(self, observe: Callable[[GameState], Any]):
        ...
        while obs_fringe:
            ...
            for action in self.actions:
                ...
                assert (
                    action not in graph[current_obs]
                    or graph[current_obs][action] == neighbor_obs
                )
                graph[current_obs][action] = neighbor_obs

                # Just validating the r(s,a,s')=r(s') assumption
                assert (
                    neighbor_obs not in reward_graph
                    or reward_graph[neighbor_obs] == reward
                )
...
class DeterministicGraph(MDPGraph):
    def __init__(
        self,
        game,
        graph: Dict[Statelike, Dict[Action, Statelike]],
    ...
    def _construct_transition_matrix(self, graph):
        ...
        action_to_idx = {
            action: action_i for action_i, action in enumerate(self.actions)
        }
    ...
    def as_transition_matrix(self) -> "np.ndarray[S,A,S]":
        ...
        T = np.zeros(
            (self.num_states, self.num_actions, self.num_states), dtype=np.float
        )

        state_to_idx = {
            state: state_i for state_i, state in enumerate(self.graph.keys())
        }
        action_to_idx = {
            action: action_i for action_i, action in enumerate(self.actions)
        }
    ...
    def as_reward_vector(self) -> "np.ndarray[S]":
        rewards = np.fromiter(
            (self.reward_graph[s] for s in self.states()), dtype=np.float
        )
        return rewards
    ...
    def as_indexed_networkx_graph(self):
        ...
        graph.add_edges_from(
            (
                (u, self.transitions[u, a], {"action": a})

...
from src.vgdl.core import BasicGame, BasicGameLevel
from src.vgdl.ontology import GridPhysics
from src.vgdl.tools import PrettyDict
...
class OrientationObserver(StateObserver):
    def __init__(self, game: BasicGame) -> None:
        super().__init__(game)
        from src.vgdl.ontology import OrientedAvatar
...
class ResourcesObserver(StateObserver):
    ...
    def get_observation(self):
        ...
        avatar = self.game.sprite_registry.get_avatar()
        resources = {
            key: avatar.resources.get(key, 0)
            for key in self.game.domain.notable_resources
        }

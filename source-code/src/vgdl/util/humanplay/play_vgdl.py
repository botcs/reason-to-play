...
from vgdl.util.humanplay.human import HumanVGDLController
...
def register_vgdl_env(domain_file, level_file, observer=None, blocksize=None):
    ...
    register(
        ...
        kwargs={
            ...
            "obs_type": observer or "image",  # 'features',
        },
...
def main():
    ...
    parser.add_argument(
        "--levelfile",
        type=str,
        default="<LOCAL-DEV-PATH>/Research/Scratch/py-vgdl/vgdl/games/sokoban_lvl0.txt",
    )
    parser.add_argument("--domainfile", "-d", type=str, default=None)
    parser.add_argument(
        "--ontology", "-m", type=str, help="Python module for the game description"
    )
    parser.add_argument(
        "--observer",
        "-s",
    ...
    parser.add_argument(
        "--blocksize", "-b", default=24, type=int, help="Render block size [default 24]"
    )
    parser.add_argument("--tracedir", type=str)
    parser.add_argument(
        "--pause_on_finish", dest="pause_on_finish", action="store_true", default=False
    )
    ...
    if args.domainfile is None:
        ...
        args.domainfile = os.path.join(
            os.path.dirname(args.levelfile),
            os.path.basename(args.levelfile).split("_")[0] + ".txt",
        )

    env_name = register_vgdl_env(
        args.domainfile, args.levelfile, observer_cls, args.blocksize
    )

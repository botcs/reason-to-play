...
KEYS_TO_ATARI_ACTION = {
    ...
    (PygletKeys.RIGHT,): "RIGHT",
    (PygletKeys.LEFT,): "LEFT",
    (PygletKeys.DOWN,): "DOWN",
    (PygletKeys.UP,): "UP",
...
class AtariControls(Controls):
    def __init__(self, action_selection):
        ...
        self.available_actions = dict(
            zip(action_selection, range(len(action_selection)))
        )
        # Maps a key combo to a Gym action (an action index)
        self.keys_to_action = {
            ATARI_ACTION_TO_KEYS[name]: code
        ...
        self.activated = {
            key: False
            for name, key in vars(PygletKeys).items()
...
class VGDLControls(Controls):
    def __init__(self, action_selection: List[str]):
        ...
        self.available_actions: Dict[str, int] = dict(
            zip(action_selection, range(len(action_selection)))
        )
        # Maps a key combo to a Gym action (an action index)
        ...
        self.activated = {
            key: False
            for name, key in vars(PygameKeys).items()
        ...
        self.special_throttle_time = 0.25

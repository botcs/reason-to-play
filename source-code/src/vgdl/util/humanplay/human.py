...
class HumanController:
    def __init__(self, env_name, trace_path=None, fps=15):
        ...
        if (
            ...
            and importlib.util.find_spec("gym_recording") is not None
        ):
    ...
    def debug(self, *args, **kwargs):
        ...
        all = dict(
            env=env, game=game, observer=observer, obs=obs, sprites=sprites, state=state
        )
        print(all)
...
class ReplayVGDLController(HumanController):
    def __init__(self, env_name, replay_actions, spy_func=None, *args, **kwargs):
        ...
        self.controls = ReplayVGDLControls(
            self.env.unwrapped.get_action_meanings(), replay_actions
        )
        self.env.render(mode="human")

    def after_step(self, step):
        if self.spy_func is not None:
            actual_action = self.env.unwrapped._action_keys[

...
class AStarWorld(object):
    def __init__(self, game):
        ...
        self.empty = [
            VGDLSprite(pos, (self.game.block_size, self.game.block_size))
    ...
    def neighbor_nodes_of_sprite(self, sprite):
        ...
        for tilex, tiley in tiles:
            if (
                tilex >= 0
                and tilex < self.game.width
                and tiley >= 0
                and tiley < self.game.height
            ):
                index = self.get_index(tilex, tiley)
    ...
    def search(self, start, goal):
        ...
        while len(openset) > 0:
            ...
            for neighbor in self.neighbor_nodes(current):
                ...
                if (
                    self.nodeInSet(neighbor, closedset)
                    and temp_g >= g_score[neighbor.index]
                ):
                    continue
                if (
                    not self.nodeInSet(neighbor, openset)
                    or temp_g < g_score[neighbor.index]
                ):
                    ...
                    f_score[neighbor.index] = g_score[neighbor.index] + self.h(
                        neighbor, goal
                    )

...
from src.vgdl.tools import unit_vector
from .constants import *
from .physics import ContinuousPhysics
...
class Flicker(VGDLSprite):
    ...
    color = RED
    limit = 1
    state_attributes = VGDLSprite.state_attributes + ["_age"]
...
class SpawnPoint(SpriteProducer):
    ...
    color = BLACK
    is_static = True
    state_attributes = VGDLSprite.state_attributes + ["counter"]
    ...
    def update(self, game):
        if (
            game.time % self.cooldown == 0
            and game.random_generator.random() < self.prob
        ):
            game.add_sprite_creation(self.stype, (self.rect.left, self.rect.top))
            self.counter += 1
...
class OrientedSprite(VGDLSprite):
    ...
    def _draw(self, game):
        ...
        if self.draw_arrow:
            ...
            pygame.draw.polygon(
                game.screen, col, triPoints(self.rect, unit_vector(self.orientation))
            )
...
class RandomMissile(Missile):
    def __init__(self, **kwargs):
        Missile.__init__(
            self,
            orientation=game.random_generator.choice(BASEDIRS),
            speed=game.random_generator.choice([0.1, 0.2, 0.4]),
            **kwargs,
        )
...
class ErraticMissile(Missile):
    ...
    def __init__(self, prob=0.1, **kwargs):
        Missile.__init__(
            self, orientation=game.random_generator.choice(BASEDIRS), **kwargs
        )
        self.prob = prob
        self.is_stochastic = prob > 0 and prob < 1
...
class AStarChaser(RandomNPC):
    from src.vgdl.ai import AStarWorld

...
from src.vgdl.core import Resource, Effect
from src.vgdl.tools import once_per_step, unit_vector
from src.vgdl.ontology.physics import *
...
def cloneSprite(sprite, partner, game):
    game.add_sprite_creation(sprite.name, (sprite.rect.left, sprite.rect.top))


def transformTo(sprite, partner, game, stype="wall"):
    from src.vgdl.ontology.sprites import OrientedSprite

    # game.destroy_sprite(sprite)
    last_rect_killed = sprite.lastrect
    game.kill_sprite(sprite)
    new_sprite = game.add_sprite_creation(stype, sprite.rect.topleft)
    if new_sprite is not None:
        new_sprite.lastrect = last_rect_killed
        assert new_sprite, "This really was not supposed to happen, cannot recover"
        if isinstance(sprite, OrientedSprite) and isinstance(
            new_sprite, OrientedSprite
        ):
            new_sprite.orientation = sprite.orientation


def stepBack(sprite, partner, game, no_symmetry=False):
    """Revert last move."""
    if partner not in game.kill_list and sprite not in game.kill_list:
        if sprite.rect == sprite.lastrect and not no_symmetry:
            partner.rect = partner.lastrect
            step_back_pusher(partner, depth=0)
        else:
            sprite.rect = sprite.lastrect
            # undo the pushing if there is one
            step_back_pusher(sprite, depth=0)


def stepBackIfHasLess(
    sprite, partner, game, resource, limit=1, no_symmetry=False, exhaustStype=None
):
    """Revert last move only if sprite has less than limit of resource.
    If sprite has enough, kill the partner instead (e.g. door opens with key).
    If exhaustStype is set, transform the partner into that type instead of killing it."""
    if sprite.resources[resource] < limit:
        stepBack(sprite, partner, game, no_symmetry=no_symmetry)
    else:
        if exhaustStype:
...
def step_back_pusher(sprite, depth):
    if depth > 5:
        return
    if sprite.just_pushed:
        sprite.just_pushed.rect = sprite.just_pushed.lastrect
        step_back_pusher(sprite.just_pushed, depth=depth + 1)
...
def find_origin_mvt(partner, depth):
    if partner.just_pushed and depth < 3:
        return find_origin_mvt(partner.just_pushed, depth + 1)
    else:
...
def bounceForward(sprite, partner, game):
    ...
    pushed_dir = find_origin_mvt(partner, depth=0)
    if abs(pushed_dir.x) + abs(pushed_dir.y) == 0:
        # the bouncing occurs the other way around
        pushed_dir = find_origin_mvt(sprite, depth=0)
        partner.physics.active_movement(partner, unit_vector(pushed_dir))
        partner.just_pushed = sprite
    else:
        sprite.physics.active_movement(sprite, unit_vector(pushed_dir))
        sprite.just_pushed = partner
    # game._updateCollisionDict(sprite)


def catapultForward(sprite, partner, game, exhaustStype=None):
    """Propel sprite one tile forward in its direction of approach.
    If exhaustStype is set, transform the catapult into that type after a successful launch."""
    if sprite.lastrect.colliderect(partner.rect):
        return  # already on catapult, don't re-launch
    direction = sprite.lastdirection
    if direction.length() == 0:
        return  # no movement direction, nothing to catapult
    direction = unit_vector(direction)
    gridsize = sprite.rect.width
    new_rect = sprite.rect.move(
        int(direction[0]) * gridsize, int(direction[1]) * gridsize
    )
    # Don't launch out of bounds
    if (
        new_rect.left < 0
        or new_rect.top < 0
        or new_rect.right > game.screensize[0]
        or new_rect.bottom > game.screensize[1]
    ):
        return
...
def turnAround(sprite, partner, game):
    ...
    sprite.physics.active_movement(sprite, DOWN, speed=1)
    # sprite.lastmove = sprite.cooldown
...
def reverseDirection(sprite, partner, game, with_step_back=True):
    if with_step_back:
        sprite.rect = sprite.lastrect
    try:
        sprite.orientation = Vector2(-sprite.orientation[0], -sprite.orientation[1])
    except:
        pass
...
def wallBounce(sprite, partner, game, friction=0):
    ...
    sprite.speed *= 1.0 - friction
    stepBack(sprite, partner, game)
    if abs(sprite.rect.centerx - partner.rect.centerx) > abs(
        sprite.rect.centery - partner.rect.centery
    ):
...
def wallStop(sprite, partner, game, friction=0):
    ...
    same_vertical = (
        partner.rect.left < sprite.rect.right < partner.rect.right
        or partner.rect.left < sprite.rect.left < partner.rect.right
    )
...
def killIfFromAbove(sprite, partner, game):
    ...
    stepBack(sprite, partner, game, no_symmetry=True)
    if sprite.rect.colliderect(partner):
        if (
...
def collectResource(sprite, partner, game):
    ...
    partner.resources[r] = max(
        0,
        min(
            partner.resources[r] + sprite.value,
            game.domain.resources_limits.get(r, float("+inf")),
        ),
    )
...
def changeResource(sprite, partner, game, resource, value=1):
    ...
    game.resource_changes.append((sprite, resource, value))
    # sprite.resources[resource] = max(0, min(sprite.resources[resource]+value, game.domain.resources_limits.get(resource, float('+inf'))))


def addResource(sprite, partner, game, resource, value=1):
    """Increments a specific resource type in sprite"""
    game.resource_changes.append((partner, resource, value))
    game.kill_list.append(sprite)
    # sprite.resources[resource] = max(0, min(sprite.resources[resource]+value, game.domain.resources_limits.get(resource, float('+inf'))))


def removeResource(sprite, partner, game, resource, value=-1):
    """Increments a specific resource type in sprite"""
    game.resource_changes.append((partner, resource, value))
    game.kill_list.append(sprite)
    # sprite.resources[resource] = max(0, min(sprite.resources[resource]+value, game.domain.resources_limits.get(resource, float('+inf'))))
...
def spawnIfHasMore(sprite, partner, game, resource, stype, limit=1):
    ...
    if sprite.resources[resource] >= limit:
        game.add_sprite_creation(stype, (sprite.rect.left, sprite.rect.top))
...
def killBothIfBHasLess(sprite, partner, game, resource, limit=1):
    """first always dies, second dies if has less than limit"""
    killSprite(sprite, partner, game)
    if partner.resources[resource] <= limit:
        killSprite(partner, sprite, game)
...
def wrapAround(sprite, partner, game, offset=0):
    ...
    elif sprite.rect.top + sprite.rect.size[1] > game.screensize[1]:
        sprite.rect.top = 0
    if sprite.rect.left < 0:
        sprite.rect.left = game.screensize[0] - sprite.rect.size[0]
    elif sprite.rect.left + sprite.rect.size[0] > game.screensize[0]:
        sprite.rect.left = 0
    sprite.lastmove = 0
...
def teleportToOther(sprite, partner, game, exhaustStype=None):
    """Teleport sprite to another sprite of the same type as partner, excluding partner.
    For bidirectional teleporter pairs where both endpoints share the same sprite type.
    Only triggers when the sprite STEPS ONTO the portal (was not already on it).
    If exhaustStype is set, both endpoints are transformed into that type after teleport."""
    # Prevent ping-pong: if the sprite started this tick already on the portal, skip.
    if sprite.lastrect.colliderect(partner.rect):
        return
    siblings = [s for s in game.sprite_registry.group(partner.key) if s is not partner]
    if not siblings:
        raise ValueError(
            f"teleportToOther: no sibling found for {partner.key} at {partner.rect.topleft}"
        )
    e = game.random_generator.choice(siblings)
